...
def phi_1D(xx, nu=torch.tensor(1.0), theta0=torch.tensor(1.0),
    ...
    if gamma < 0 and torch.isinf(torch.exp(-2 * gamma)):
        Qadjust = -2 * gamma
    ...
    integrand = lambda xi: torch.exp(-4 * gamma * h * xi - 2 * gamma * (1 - 2 * h) * xi ** 2
                                     - Qadjust)
    int0, eps = scipy.integrate.quad(integrand, 0, 1, epsabs=0,
                                     points=torch.linspace(0, 1, 41))

    ints = torch.empty(len(xx))
    ...
    if gamma < 0:
        ...
        for ii, q in enumerate(xx):
            val, eps = scipy.integrate.quad(integrand, q, 1, epsabs=0,
                                            points=torch.linspace(q, 1, 41))
            ints[ii] = val
        phi = torch.exp(4 * gamma * h * xx + 2 * gamma * (1 - 2 * h) * xx ** 2) * ints / int0
    else:
        # In this case, the prefactor may be divergent, so we do the integral
        # with the prefactor pulled inside
        integrand = lambda xi, q: torch.exp(-4 * gamma * h * (xi - q) -
...
def phi_1D_genic(xx, nu=torch.tensor(1.0), theta0=torch.tensor(1.0), gamma=torch.tensor(0), theta=None,
                 beta=torch.tensor(1)):
    """
    One-dimensional phi for a constant-sized population with genic selection.
...
def phi_1D_snm(xx, nu=torch.tensor(1.0), theta0=torch.tensor(1.0), theta=None, beta=torch.tensor(1.0)):
    """
