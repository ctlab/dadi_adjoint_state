...
def TDMAsolver(a, b, c, d):
    ...
    ac, bc, cc, dc = map(torch.clone, (a, b, c, d))  # copy arrays
...
def _compute_dt(dx, nu, ms, gamma, h):
    ...
    maxVM = max(0.25 / nu, sum(ms), \
                abs(gamma) * 2 * max(torch.abs(h + (1 - 2 * h) * 0.5) * 0.5 * (1 - 0.5),
                                     torch.abs(h + (1 - 2 * h) * 0.25) * 0.25 * (1 - 0.25)))
    if maxVM > 0:
...
def one_pop(phi, xx, T, nu=torch.tensor(1.0), gamma=torch.tensor(0), h=torch.tensor(0.5), theta0=torch.tensor(1.0),
            initial_t=0, frozen=False, beta=torch.tensor(1)):
    """
    ...
    while current_t < T:
        ...
        if torch.any(torch.tensor([torch.less(T, 0), torch.less(nu, 0), torch.less(theta0, 0)])):
            raise ValueError('A time, population size, migration rate, or '
                             'theta0 is < 0. Has the model been mis-specified?')
        if torch.any(torch.tensor(torch.equal(nu, torch.zeros((1,), dtype=torch.float64)))):
        ...
        # matrices there.
        phi = Integration1D.implicit_1Dx(phi, xx, nu, gamma, h, beta, this_dt,
                                         use_delj_trick=use_delj_trick)
        current_t = next_t
    return phi, phi_inj


def two_pops(phi, xx, T, nu1=torch.tensor(1.0), nu2=torch.tensor(1.0), m12=torch.tensor(0), m21=torch.tensor(0),
             gamma1=torch.tensor(0), gamma2=torch.tensor(0), h1=torch.tensor(0.5), h2=torch.tensor(0.5),
             theta0=torch.tensor(1.0), initial_t=0, frozen1=False, frozen2=False, nomut1=False, nomut2=False,
             enable_cuda_cached=False):
    """
    Integrate a 2-dimensional phi forward.
    ...
    if torch.all([var.ndim == 0 for var in vars_to_check]):  # why there was "if False and .."?
    ...
    #                                                           nu1_f, nu2_f, m12_f, m21_f, gamma1_f, gamma2_f, h1_f,
    #                                                           h2_f, theta0_f,
    #                                                           frozen1, frozen2, nomut1, nomut2)
    #     return phi
    ...
    dx, dy = torch.diff(xx), torch.diff(yy)
    while current_t < T:
        ...
        if torch.any(torch.tensor([torch.less(T, 0), torch.less(nu1, 0), torch.less(nu2, 0), torch.less(m12, 0),
                                   torch.less(m21, 0), torch.less(theta0, 0)])):
            raise ValueError('A time, population size, migration rate, or theta0 '
                             'is < 0. Has the model been mis-specified?')
        if torch.any(
                torch.tensor([torch.equal(nu1, torch.zeros((1,))), torch.equal(nu2, torch.zeros((1,)))])):
            # torch.equal(nu1, torch.zeros((1,), dtype=torch.float64))
            raise ValueError('A population size is 0. Has the model been '
                             'mis-specified?')
...
def three_pops(phi, xx, T, nu1=1, nu2=1, nu3=1,
    ...
    if cuda_enabled:
        ...
        phi = dadi.cuda.Integration._three_pops_temporal_params(phi, xx, T, initial_t,
                                                                nu1_f, nu2_f, nu3_f, m12_f, m13_f, m21_f, m23_f, m31_f,
                                                                m32_f,
...
def four_pops(phi, xx, T, nu1=1, nu2=1, nu3=1, nu4=1,
    ...
    if cuda_enabled:
        ...
        phi = dadi.cuda.Integration._four_pops_temporal_params(phi, xx, T, initial_t,
                                                               nu1_f, nu2_f, nu3_f, nu4_f, m12_f, m13_f, m14_f, m21_f,
                                                               m23_f, m24_f, m31_f, m32_f, m34_f,
                                                               m41_f, m42_f, m43_f, gamma1_f, gamma2_f, gamma3_f,
                                                               gamma4_f, h1_f, h2_f, h3_f, h4_f,
...
def five_pops(phi, xx, T, nu1=1, nu2=1, nu3=1, nu4=1, nu5=1,
    ...
    if (frozen1 and (m12 != 0 or m21 != 0 or m13 != 0 or m31 != 0 or m41 != 0 or m14 != 0 or m15 != 0 or m51 != 0)) \
            or (
            frozen2 and (m12 != 0 or m21 != 0 or m23 != 0 or m32 != 0 or m24 != 0 or m42 != 0 or m25 != 0 or m52 !=
                         0)) \
            or (
            frozen3 and (m13 != 0 or m31 != 0 or m23 != 0 or m32 != 0 or m34 != 0 or m43 != 0 or m35 != 0 or m53 !=
                         0)) \
            or (
            frozen4 and (m14 != 0 or m41 != 0 or m24 != 0 or m42 != 0 or m34 != 0 or m43 != 0 or m45 != 0 or m54 !=
                         0)) \
            or (
            frozen5 and (m15 != 0 or m51 != 0 or m25 != 0 or m52 != 0 or m35 != 0 or m53 != 0 or m45 != 0 or m54 != 0)):
        raise ValueError('Population cannot be frozen and have non-zero '
                         'migration to or from it.')
    bb = aa = zz = yy = xx
    ...
    m12_f, m13_f, m14_f, m15_f = Misc.ensure_1arg_func(m12), Misc.ensure_1arg_func(m13), Misc.ensure_1arg_func(
        m14), Misc.ensure_1arg_func(m15)
    m21_f, m23_f, m24_f, m25_f = Misc.ensure_1arg_func(m21), Misc.ensure_1arg_func(m23), Misc.ensure_1arg_func(
        m24), Misc.ensure_1arg_func(m25)
    m31_f, m32_f, m34_f, m35_f = Misc.ensure_1arg_func(m31), Misc.ensure_1arg_func(m32), Misc.ensure_1arg_func(
        m34), Misc.ensure_1arg_func(m35)
    m41_f, m42_f, m43_f, m45_f = Misc.ensure_1arg_func(m41), Misc.ensure_1arg_func(m42), Misc.ensure_1arg_func(
        m43), Misc.ensure_1arg_func(m45)
    m51_f, m52_f, m53_f, m54_f = Misc.ensure_1arg_func(m51), Misc.ensure_1arg_func(m52), Misc.ensure_1arg_func(
        m53), Misc.ensure_1arg_func(m54)
    theta0_f = Misc.ensure_1arg_func(theta0)
    ...
    if cuda_enabled:
        ...
        phi = dadi.cuda.Integration._five_pops_temporal_params(phi, xx, T, initial_t,
                                                               ...
                                                               m12_f, m13_f, m14_f, m15_f, m21_f, m23_f, m24_f, m25_f,
                                                               m31_f, m32_f, m34_f, m35_f,
                                                               m41_f, m42_f, m43_f, m45_f, m51_f, m52_f, m53_f, m54_f,
                                                               gamma1_f, gamma2_f, gamma3_f, gamma4_f, gamma5_f,
                                                               h1_f, h2_f, h3_f, h4_f, h5_f, theta0_f, frozen1, frozen2,
                                                               frozen3, frozen4, frozen5)
        return phi

    current_t = initial_t
    nu1, nu2, nu3, nu4, nu5 = nu1_f(current_t), nu2_f(current_t), nu3_f(current_t), nu4_f(current_t), nu5_f(current_t)
    gamma1, gamma2, gamma3, gamma4, gamma5 = gamma1_f(current_t), gamma2_f(current_t), gamma3_f(current_t), gamma4_f(
        current_t), gamma5_f(current_t)
    ...
    while current_t < T:
        ...
        gamma1, gamma2, gamma3, gamma4, gamma5 = gamma1_f(next_t), gamma2_f(next_t), gamma3_f(next_t), gamma4_f(
            next_t), gamma5_f(next_t)
...
def _compute_delj(dx, MInt, VInt, axis=1):
    r"""
    ...
    if use_delj_trick:
        ...
        # upslice = [None for ii in range(MInt.ndim)]
        # upslice[axis] = slice(None)

        # wj = 2 * MInt * dx[upslice] torch.unsqueeze(d, 0)
        wj = 2 * MInt * torch.unsqueeze(dx, axis)
        epsj = torch.exp(wj / torch.unsqueeze(VInt, axis))
        delj = (-epsj * wj + epsj * torch.unsqueeze(VInt, axis) - torch.unsqueeze(VInt, axis)) / (wj - epsj * wj)
        # These where statements filter out edge case for delj
        delj = torch.where(torch.isnan(delj), 0.5, delj)
        delj = torch.where(torch.isinf(delj), 0.5, delj)
    else:
        delj = 0.5
    return delj


def _one_pop_const_params(phi, xx, T, nu=torch.tensor(1), gamma=torch.tensor(0), h=torch.tensor(0.5),
                          theta0=torch.tensor(1.0),
                          initial_t=0, beta=torch.tensor(1)):
    """
    ...
    if torch.any(torch.tensor([torch.less(T, 0), torch.less(nu, 0), torch.less(theta0, 0)])):
        raise ValueError('A time, population size, migration rate, or theta0 '
                         'is < 0. Has the model been mis-specified?')
    if torch.any(
            torch.tensor(torch.equal(nu, torch.zeros((1,), dtype=torch.float64)))):  # s(0, dtype=torch.float32)))):
        raise ValueError('A population size is 0. Has the model been '
                         'mis-specified?')
    ...
    a = torch.zeros(phi.shape)
    a[1:] += dfactor[1:] * (-MInt * delj - V[:-1] / (2 * dx))

    c = torch.zeros(phi.shape)
    c[:-1] += -dfactor[:-1] * (-MInt * (1 - delj) + V[1:] / (2 * dx))

    b = torch.zeros(phi.shape)
    ...
    dt = _compute_dt(dx, nu, torch.zeros(1), gamma, h)
    current_t = initial_t
    # outputs_phi = []
    # outputs_phi_inj = []
    # phi = phi_initial
    ...
    while current_t < T:
        ...
        current_t += this_dt
    # result_phi = torch.stack(outputs_phi, dim=0)
...
def _two_pops_const_params(phi, xx, T, nu1=1, nu2=1, m12=0, m21=0,
    ...
    Mx = _Mfunc2D(torch.unsqueeze(xx, 1), torch.unsqueeze(yy, 0), m12, gamma1, h1)
    MxInt = _Mfunc2D((torch.unsqueeze(xx[:-1], 1) + torch.unsqueeze(xx[1:], 1)) / 2, torch.unsqueeze(yy, 0), m12,
                     gamma1, h1)

    Vy = _Vfunc(yy, nu2)
    VyInt = _Vfunc((yy[1:] + yy[:-1]) / 2, nu2)
    My = _Mfunc2D(torch.unsqueeze(yy, 0), torch.unsqueeze(xx, 1), m21, gamma2, h2)
    MyInt = _Mfunc2D((torch.unsqueeze(yy[1:], 0) + torch.unsqueeze(yy[:-1], 0)) / 2, torch.unsqueeze(xx, 1), m21,
                     gamma2, h2)
    ...
    deljy = _compute_delj(dy, MyInt, VyInt, axis=0)

    # The nuax's here broadcast the our various arrays to have the proper shape
    # to fit into ax,bx,cx
    ax, bx, cx = [torch.zeros(phi.shape) for ii in range(3)]
    ax[1:] += torch.unsqueeze(dfact_x[1:], 1) * (-MxInt * deljx - torch.unsqueeze(Vx[:-1], 1) / (2 * torch.unsqueeze(
        dx, 1)))
    cx[:-1] += torch.unsqueeze(dfact_x[:-1], 1) * (MxInt * (1 - deljx) - torch.unsqueeze(Vx[1:], 1) / (2 * dx[:,
    nuax]))
    bx[:-1] += torch.unsqueeze(dfact_x[:-1], 1) * (MxInt * deljx + torch.unsqueeze(Vx[:-1], 1) / (2 *
                                                                                                  torch.unsqueeze(dx,
                                                                                                                  1)))
    bx[1:] += torch.unsqueeze(dfact_x[1:], 1) * (-MxInt * (1 - deljx) +
                                                 torch.unsqueeze(Vx[1:], 1) / (2 * torch.unsqueeze(dx, 1)))
    ...
    ay, by, cy = [torch.zeros(phi.shape) for ii in range(3)]
    ay[:, 1:] += dfact_y[1:] * (-MyInt * deljy - torch.unsqueeze(Vy[:-1], 0) / (2 * dy))
    cy[:, :-1] += dfact_y[:-1] * (MyInt * (1 - deljy) - torch.unsqueeze(Vy[1:], 0) / (2 * dy))
    by[:, :-1] += dfact_y[:-1] * (MyInt * deljy + torch.unsqueeze(Vy[:-1], 0) / (2 * dy))
    by[:, 1:] += dfact_y[1:] * (-MyInt * (1 - deljy) + torch.unsqueeze(Vy[1:], 0) / (2 * dy))
    ...
    # if cuda_enabled:
    #     phi, phi_inj = _two_pops_const_params(phi, xx, theta0, frozen1, frozen2, nomut1, nomut2, ax, bx, cx, ay,
    ...
    while current_t < T:
        ...
        phi_inj = phi
        if not frozen1:
            phi = Integration2D.implicit_precalc_2Dx(phi, ax, bx, cx, this_dt)
        if not frozen2:
            phi = Integration2D.implicit_precalc_2Dy(phi, ay, by, cy, this_dt)
        current_t += this_dt
