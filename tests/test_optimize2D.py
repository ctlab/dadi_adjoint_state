...
import numpy as np
from dadi_torch import Demographics2D
import time
import os
from models import simulation2D
...
class TestOptimizeParamsBFGS(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        os.chdir(os.path.dirname(os.path.abspath(Demographics2D.__file__)))
    ...
    def test_optimize_two_epoch_ASM(self):
        ...
        for i in range(0, P.shape[0]):
            ...
            func = Demographics2D.two_epoch_ASM
    ...
    def test_optimize_three_epoch_ASM(self):
        ...
        for i in range(0, P.shape[0]):
            # nu, gamma, h, beta, theta0 = P[i][0], P[i][1], P[i][2], P[i][3], P[i][4]
            func = Demographics2D.three_epoch_ASM
            child_logger.info('Initial parameters P[{}]={}'.format(i, str(P[i])))
            t1 = time.time()
...
class TestOptimizeParamsGradDescent(unittest.TestCase):
    ...
    def test_optimize_bottlegrowth(self):
        child_logger = logging.getLogger("test_optimize_grad_bottlegrowth")
        child_logger.addHandler(logging.FileHandler(log_file))
        child_logger.setLevel(10)
        P = np.asarray([[[50.], [50.], [90]]]) # nuB,nuF,T = params
        upper_bound = [100., 100., 100.]
        lower_bound = [1., 1., 1.]
        data_bottlegrowth = dadi.Spectrum.from_file('fs_data_bottlegrowth_2d.fs')  # 'fs_data_two_epoch_ASM.fs')
        ns = data_bottlegrowth.sample_sizes
        print("data", data_bottlegrowth, "\nns=", ns)
        # print("P_scaled.shape[0]", P_scaled.shape[0])
        for i in range(0, P.shape[0]):
            model_func = Demographics2D.bottlegrowth  # fs
            child_logger.info('Initial parameters P[{}]={}'.format(i, str(P[i])))
            t1 = time.time()
            print("P[i]={}".format(P[i]))
            adjointer = asm_torch.AdjointStateMethod(P[i], data_bottlegrowth, model_func, self.pts, upper_bound,
                                                     lower_bound)
