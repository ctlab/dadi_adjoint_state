...
class OptimizeParamsTestCase(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        cls.P = np.asarray([[0.218, 0.60, 0.63, 7.09, 1.]])
    ...
    def test_optimize(self):
        for i in range(0, self.P.shape[0]):
            ...
            childLogger.info('Initial parameters P[{}]={}'.format(i, str(self.P[i])))
            ...
            t1 = time.time()
            popt_asm = dadi.Inference.optimize_fgrad(self.P[i], self.data, func, self.pts,
                                                     lower_bound=self.lower_bound,
                                                     upper_bound=self.upper_bound,
                                                     verbose=1, flush_delay=0.5, epsilon=1e-3,
                                                     gtol=1e-5, multinom=True, maxiter=None, full_output=False,
                                                     func_args=[], func_kwargs={'xx': dadi.Numerics.default_grid,
                                                                                'initial_t': 0,
                                                     'final_t': 3}, fixed_params=None, ll_scale=1,
                                                     output_file=log_file)
