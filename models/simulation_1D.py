...
def sim_two_epoch_ASM(ns, pts):
    params = [4]  # nu
    T = 2
    model = Demographics1D.two_epoch_ASM(params, ns, pts, T=T, xx=dadi.Numerics.default_grid, initial_t=0)
    Nanc = 100
    theta = 4 * mu * L * Nanc  # mutation flux
    print("theta", theta)
    data = model * theta
...
if __name__ == "__main__":
    ...
    data, model = sim_three_epoch_ASM(ns, pts)
    write_fs("three_epoch_ASM")
    calc_Nanc(model, data, "three_epoch_ASM")
